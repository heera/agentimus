  ...
  data() {
    return {
      ...
      featureGroups: [
        {
          ...
          items: [
            ...
            { name: 'Shareable scorecard', where: 'Settings → Share', desc: 'Gives the score a public face — everything off by default. One switch serves a scorecard page at an address you choose, an embeddable badge, and a preview card image that shows your score when the link is shared. Show the number, or only the earned “AI-Ready” mark — a fixed bar at 90, the same on every site, and below it the surfaces honestly say “working toward AI-Ready” with no number anywhere. Print your site’s name or just your domain; pick the accent, badge text and needs-work colors and the badge’s shape — one scheme across badge, card and page. The public page has its own switch beneath the master: off, its address is a normal 404 while the badge and card keep serving. Sharing always opens a preview first, with a link you can change; nothing is ever posted for you, and what’s shown is always the live, current score.', tag: 'Opt-in' },
        ...
        },
        {
          ...
          items: [
            { name: 'Writing assistant', where: 'the pen button, every Agentimus screen', desc: 'Describe the post you want; the assistant proposes an outline you can edit, then writes every section of the article in parallel — the outline fills in as parts land, long posts aren’t capped by a single AI response, and a failed section retries alone. The assembled draft — real editor blocks, AI description, topics, suggested categories and tags — is shown to you before a single thing is saved. Create draft opens the post in the editor, where image placeholders arrive with their alt text filled in: a Generate button turns any image block’s alt text into an image, a Featured image (AI) panel drafts the featured image from the title, and Ask AI rewrites, shortens or extends any text block. It also revises an existing post you pick — content changes, a post’s status never does, and WordPress revisions keep every prior version. Needs “Let connected agents write” plus an AI provider; it never publishes.', tag: 'Opt-in' },
        ...
        },
        {
          ...
          items: [
            ...
            { name: 'MCP server', where: '/wp-json/agentimus/v1/mcp', desc: 'Off by default. One switch (Settings → Discovery) runs a Model Context Protocol server on your own site — everything needed ships with the plugin — so AI tools you already use (Claude Code, Claude Desktop, Codex) can run the nine read-only Agentimus tools above; the switch card writes the exact setup for each. A second switch (off by default) adds the write tools — draft/edit content, set AI topics and descriptions, apply Readiness fixes — and a third decides whether agents may publish or only leave drafts for review. Nothing is public: every call signs in with a WordPress login, each tool keeps the same permission checks as its admin screen, every call is recorded under Agent Access, and turning the switch off disconnects connected tools immediately.', tag: 'Opt-in' },
        ...
        },
        {
          ...
          items: [
            ...
            { name: 'XML sitemap', where: '/agentimus-sitemap.xml', desc: 'A fallback sitemap — served only when neither WordPress core nor an SEO plugin provides one.', tag: 'On' },
          ],
        },
        {
        ...
        },
        {
          ...
          items: [
            ...
            { name: 'Weekly email', where: 'your inbox · Settings → Weekly email', desc: 'A short email once a week: agent visits against the week before, readers arriving from AI answers, impostors caught, your readiness score and its move since the last note, and one thing worth doing. Built only from the local data above and sent with WordPress’s own mail — no images, no tracking. A week with nothing to report sends nothing, and every email ends with a one-click stop link.', tag: 'On' },
      ...
      faqs: [
        ...
        { q: 'Can I show my score publicly?', a: 'Yes — Settings → Share, all off by default. One switch publishes three things your own site generates: a scorecard page at an address you choose, a badge you can embed anywhere, and a preview card image that appears when the page’s link is shared. You choose what the public sees — the number, or only the “AI-Ready” mark, earned at a fixed score of 90 so it means the same on every site (below it, the surfaces say “working toward AI-Ready” and never show the number). The page has its own on/off: off, its address returns a normal 404 while the badge and card keep working — and the badge only links to the page while the page exists. One caution: badge code you’ve already pasted somewhere is fixed at copy time, so if you later turn the page off or change its address, re-copy and replace the old ones. What’s shown is always your live, current score — never a pinned best day.' },
        ...
        { q: 'Will this block Google or real search engines?', a: 'No. Blocking is opt-in and aimed at AI training crawlers and spoofed bots. Real search engines are recognised and never blocked by default — and with “Verify bot identities” on, a scanner that only uses a crawler’s name is told apart from the real one by the operator’s own published checks, so the genuine crawler stays welcome while the fake is refused. Anything the checks can’t settle is served, never guessed at.' },
        ...
        <div class="ar-about-priv__cell ar-about-priv__cell--head">
          ...
          <div>
            ...
            <p>
              ...
              And one email: the <strong>weekly digest</strong> — on by default, stopped with one click —
              goes only to the inbox you chose, carrying your own numbers and nothing else.
            </p>
          </div>
        </div>
