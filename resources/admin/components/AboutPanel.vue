  ...
  data() {
    return {
      ...
      featureGroups: [
        ...
        },
        {
          ...
          items: [
            ...
            { name: 'MCP server', where: '/wp-json/agentimus/v1/mcp', desc: 'Off by default. One switch (Settings → Discovery) runs a Model Context Protocol server on your own site — everything needed ships with the plugin — so AI tools you already use (Claude Code, Cursor, ChatGPT connectors) can run the nine read-only Agentimus tools above. Nothing is public: every call signs in with a WordPress login, each tool keeps the same permission checks as its admin screen, every call is recorded under Agent access, and turning the switch off disconnects connected tools immediately.', tag: 'Opt-in' },
          ],
        },
        {
      ...
      faqs: [
        ...
        { q: 'Does Agentimus run an MCP server?', a: 'Yes — as an opt-in, on WordPress 6.9 or newer. Turn on Settings → Discovery → MCP server and AI tools you already use (Claude Code, Cursor, ChatGPT connectors) can talk to your site over the Model Context Protocol and run the same nine read-only, permission-checked tools your admin AI gets. Everything needed ships with the plugin. Nothing becomes public: every request signs in with a WordPress login (an application password works), every call is recorded under More → Agent access, and turning the switch off disconnects connected tools immediately. Worth knowing: an application password isn’t scoped to this server — it signs in as that user across your whole REST API — so give each AI tool its own password, on a user with only the permissions it needs.' },
