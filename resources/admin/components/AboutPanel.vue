  ...
  data() {
    return {
      ...
      faqs: [
        ...
        { q: 'Can an agent write to my site through MCP?', a: 'Only if you say so, twice. The MCP server starts read-only; a second switch (“Let connected agents write”) adds the write tools — draft and edit posts and pages, set their AI topics and descriptions, and apply Readiness fixes (a fixed list that can only turn documented features on, never loosen a protection). Even then, agents can’t publish: they leave drafts and pending posts for your review, unless you flip a third switch that allows going live. Every write runs as the signed-in user — an agent can never do more than that user could in the editor — and every call is recorded under More → Agent access.' },
