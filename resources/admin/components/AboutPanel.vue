    ...
    return {
      ...
      screens: [
        ...
        },
        {
          ...
          facts: [
            { k: 'info', t: 'The tab’s amber count is the same number the screen’s headline states — urgent plus worth-doing, kept live from any screen. Zero hides the badge entirely: a quiet tab honestly means nothing needs you.' },
        ...
        },
        {
          ...
          purpose: 'Every request to your agent-facing files — llms.txt, the Markdown copies of your pages (the .md twins), the discovery documents — and the visits recognised AI agents make, each one named, judged and time-stamped. This is the plugin’s ground truth: when a wizard proof, a pin or a chart claims something happened, the row that proves it lives here.',
        ...
        },
        {
          ...
          purpose: 'The record of every agent that signed in with a key — every key created, every ability it used, every refusal. If the Request Log is who knocked, this is what the ones you let in actually did.',
      ...
      featureGroups: [
        ...
        },
        {
          ...
          items: [
            ...
            { name: 'Change feed', where: '/agentimus-changes.json?since=', desc: 'What changed lately — added, updated and removed content as JSON, with a ?since= filter so an agent re-reads only what changed instead of crawling the whole site again. Removals are announced honestly (tombstones), it’s one bounded, cached query, and it’s advertised in discovery.json so agents find it on their own.', tag: 'On' },
          ],
        },
        {
      ...
      faqs: [
        ...
        { q: 'Does it slow my site down?', a: 'Barely. Generated documents are cached, JSON-LD is tiny, and the plugin makes no external calls on the front end — nothing is fetched from another server while your pages load. (The optional citation checks run only in the admin or on a schedule, never during a page view.)' },
        ...
        { q: 'Where do I see what AI is doing on my site?', a: 'Three places, all on your own server. The Dashboard summarises both directions — which AI crawlers fetched your files, and how many readers AI sent you. More → Visitors is the full report on those visitors: day by day, by assistant and landing page. More → Request Log is the full report on the crawlers: one row per request, filterable by client, endpoint, network and date.' },
