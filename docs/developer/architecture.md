...
| `Abilities\Registrar` | Registers the `agentimus/*` abilities on the WordPress Abilities API, so WordPress's own admin AI — and external agents via the MCP adapter — can use them. Two tiers: twenty-nine read-only abilities (always), plus twelve write abilities (`create-content`, `update-content`, `edit-content`, `describe-image`, `write-description`, `write-topics`, `write-search-fields`, `apply-fix`, `set-aside-page`, `retry-announcement`, `review-client`, `recheck-client`) registered only while the owner's `enable_agent_writes` switch is on. Each ability carries the capability check of the screen or action it maps to; publishing needs the further `agent_writes_publish` switch. Execution lives in `Abilities\ContentWriter` (posts/pages, terms, featured image), `Abilities\ContentEditor` (the body an edit is written against, and one anchored passage of it), `Abilities\MediaWriter` (alt text on an image already in the library), `Abilities\Fixer` (readiness remediations, closed vocabulary) and `Abilities\Triage` (set-aside, one page per call — the bulk forms stay with the owner, behind their confirmation dialogs). |
