...
| `agentimus_mcp_server_abilities` | filter | `( string[] $names ): string[]` | The abilities Agentimus exposes over its own scoped MCP server (`/wp-json/agentimus/v1/mcp`) to external agents. Default: its nineteen read-only `agentimus/read-*` (and per-post) abilities, plus the five write abilities while the write tier is on. Trim to narrow what leaves the site. |
