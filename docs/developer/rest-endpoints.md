...
`agentimus/read-readiness`, `read-findings`, `read-audience`, `read-ai-visibility`, `read-ai-traffic`, `read-request-log`, `read-edge-traffic`, `read-search-visibility`, `read-google-index`, `read-search-performance`, `read-search-opportunities`, `identify-bot`, `read-content-issues`, `read-terms`, `check-page`, `preview-schema`, `preview-markdown`, `scan-exposed-files`, `suggest-internal-links`, `search-media`.
...
`agentimus/create-content`, `update-content`, `edit-content`, `describe-image`, `describe-content-image`, `write-description`, `write-topics`, `write-search-fields`, `apply-fix`, `set-aside-page`, `retry-announcement`, `review-client`, `recheck-client`.

The write tools run as the authenticated user through core's own write paths — `create-content` gates on the post type's `create_posts` capability, the per-post tools on `edit_post`, `apply-fix` and `set-aside-page` on `manage_options`; categories/tags follow the taxonomy's `assign_terms`/`edit_terms` caps (free-form tags creatable with assign, new categories need manage — wp-admin's own distinction) and a featured-image URL import needs `upload_files`. `status=publish` additionally requires the `agent_writes_publish` setting plus the user's own publish cap; otherwise writes are draft-first. `write-search-fields` sets the focus keyword and the SEO title — the two fields that decide how a page appears in a search result and the last two an agent could not reach; both change what the page is MEASURED against without changing a word of it, so writing either marks the page's stored verdict out of date and books the re-read, and the SEO title is refused outright where an SEO plugin owns titles rather than stored where nothing serves it. `update-content` is annotated destructive (a body replacement on a post type without revision support is unrecoverable); `apply-fix` enacts a readiness check's own remediation from a closed vocabulary that can only enable documented features — it will never flip `blog_public` or loosen a protection. `edit-content` and `describe-image` are the two that make maintenance possible rather than only authoring, and both are built around refusing rather than guessing. `edit-content` replaces ONE passage of a body: `old` must appear in the stored source exactly once — not found and found-twice are both refusals that write nothing — and three mechanical guards sit after the match, none of them a judgement call. An edit may not leave a block comment half-open (the open/close balance must survive it), may not leave the page empty, and may not run while `wp_check_post_lock()` says somebody has the post open in the editor, because their next autosave would carry the body from before the write and silently undo it. `dry_run` returns what the edit would produce without writing, the previous body is kept as a revision whose id comes back in the answer, and the saved body is read back and compared — a save the site filtered reports as a failure naming the revision, never as a success. Its anchors can only honestly come from `read-content`, which returns `post_content` byte for byte (block comments, shortcodes and entities included) — `preview-markdown` returns the .md twin, which is a rendering and cannot be edited and sent back. `describe-image` writes the alt text on an image ALREADY in the library, which nothing could do before: `featured_image_alt` on the two content writers only ever landed on the sideload path, so the commonest content flag on a real site had no fix behind it that did not duplicate the attachment. It is aimed by `post_id` (that page's featured image — what a run through the worklist has) or by `attachment_id`, never both, and it refuses an image that already carries a description unless `replace=true` is passed by a caller that has read what is there; it cannot blank one at all. Because alt text lives on the attachment, writing it saves no post and nothing would otherwise age the pages' stored verdicts, so every page using that image is marked for re-reading and their ids come back in `refreshed` — marked, not re-read, and the tool says so. `describe-content-image` is the in-content twin of `describe-image`, and the distinction is the whole reason it exists: WordPress copies an image's library description into the block at the moment it is inserted and never consults it again, so the alt a reader gets from a published page is the one in the BODY — writing the attachment's alt changes nothing on a page that already exists, which is why `describe-image` can close a `featured_alt` row and can never close an `alt_text` one. It is a body write and carries every guard `edit-content` carries (they share one private `apply()`): a builder-owned page is refused, a post somebody has open is refused, the previous body is kept as a revision and the save is read back. What it does not share is anchoring by text — the `<img>` is located and rewritten by byte offset (`BodyImages`), so there is no anchor to get wrong and no neighbouring byte to disturb; the tag's other attributes, and the rest of the document, come through identical. It is aimed with `index` (from `read-content`'s new `images` list, and never ambiguous), `src` or `attachment_id` — one of the three, or none at all on a page holding exactly one image — and an `attachment_id` that matches no `wp-image-N` class falls back to matching the file, because images placed in the classic editor carry no such class. Two refusals encode decisions rather than gaps: an image somebody has described needs `replace=true`, and so does `alt=""`, which is the standards marker for a decorative image and is deliberately not flagged by the checks — a fixing run announcing every spacer on a site would be damage. A file-name alt is neither, and is written over freely. The library copy is reported (`libraryAlt`, `libraryNeedsDescribing`) and never quietly written: filling it would change what `featured_alt` reports on every other page using that picture without marking one of them for re-reading. `search-media` gained `undescribed` for the other half of the same work — the images that need describing are exactly the ones no search matches, so nothing could enumerate them; the empty-alt pass is exact in SQL, the file-name pass reads a bounded page of described images and reports how many in `scanned`. `set-aside-page` is the triage verb behind `POST /optimize/ignore`: it marks ONE page as not-cited content so no content check grades it, or puts it back, and it takes a single post id in either direction. The screen's two bulk actions — set aside every page a check flags, restore all — deliberately have no ability: both sit behind a confirmation dialog naming the count, and a confirmation dialog is a human gate that an agent cannot be shown.
...
**Scope is enforced twice.** `Registrar::filter_tools_for_scope()` narrows the advertised tool list per request (a read-only key is shown the twenty read tools, never the six write ones), and each write ability's permission callback is wrapped by both `McpToken::gate_write_permission()` and `Oauth\Server::gate_write_permission()`. Hiding the tools is a courtesy; the gate is the boundary. Neither can exceed the site's own write settings.
