...
### All of `assets/` is git-ignored build output

Two halves, both listed in `.gitignore` and neither ever committed:
...
`assets/admin/` is the Vue admin app, bundled by Vite. `assets/*.js` are the three standalone
front-end scripts — the WebMCP bridge, the AI-referral beacon and the Focus live counter —
minified from `resources/public/` by esbuild. `npm run build` runs both steps.

The **source** of every one of them lives in `resources/`, which is what the repo keeps and
what ships in the zip beside the built files, so the build stays reproducible and the code
stays readable. Edit `resources/public/webmcp.js`, never `assets/webmcp.js`: the latter is
overwritten by the next build.

If you clone the repo and do not run the build, the admin screens have no JS/CSS and the
front-end scripts 404 — run `npm install && npm run build` first.
