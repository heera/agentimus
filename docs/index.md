...
**It lets the AI tools you already use operate your site.** An opt-in **[MCP server]({{ '/user-manual/mcp-server.html' | relative_url }})** ships inside the plugin, so Claude, Claude Code, Cursor, ChatGPT or Codex can connect and ask your site questions — readiness, AI traffic, bot activity, per-page readability. You connect one by **approving it**: give the assistant your server address, and it asks you for permission on a page on your own site, where you choose read-only or read-and-write. Behind two more opt-in switches, the same connection can **draft, edit and publish** posts and pages — categories, tags, featured image, AI topics and descriptions — and apply Readiness fixes. Every write runs as the signed-in WordPress user, checked against that user's own permissions and recorded under Agent Access. All three switches are **off by default**.
...
Turn on the built-in MCP server and the AI tools you already use — **Claude, Claude Code, Cursor, ChatGPT, Codex** — can connect to your site and work with it directly. It's the second half of the plugin, and every part of it is opt-in and off by default.
...
Every write runs as the **signed-in WordPress user**, within that user's own permissions, and is recorded under **Agent Access**. Each connected assistant is listed with its own Disconnect, so cutting one off leaves the others working. Full setup — the three-switch trust ladder, the approval flow, and how to connect each tool — is on the **[MCP server]({{ '/user-manual/mcp-server.html' | relative_url }})** page. (Prefer WordPress's own AI to draft a page's description and topics from inside the editor? That's **[Write with AI]({{ '/user-manual/write-with-ai.html' | relative_url }})**.)
