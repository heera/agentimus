...
* Improved: a network blip during the index check no longer ends the run — a slow or missing answer from Google quietly rejoins the back of the line and the check carries on, reporting a failure only after three misses in a row. The run resumes where it stopped either way, so one slow answer never costs the whole watchlist's inspection budget or asks for another press of Check now. Google's inspection also gets the time it actually takes to answer.
* Improved: In Google's Index catches up when you come back — a check keeps going server-side while the tab is hidden or the laptop sleeps, and the card re-reads where it got to, and finishes the remainder, the moment you return. No reload needed.
* Improved: In Google's Index spends its rows on what needs eyes. Problem pages sit at the top and join the daily check until they heal — up to 20 a day, the one asked about longest ago first. A page that heals says "now on Google" for about two days instead of silently vanishing. Healthy watched pages fold into one line — a green row that never changes carries no news. The headline counts speak for the whole site now, not just the watchlist — on a 200-page site with 100 pages out of the index, the first number you read says so. And the summary sentence accounts for every page, so the healthy majority is quiet, never invisible.
* Improved: every problem state on In Google's Index is a collapsible with its true count always visible — open one and its pages load on demand, 50 at a time, with real pagination at the table's foot and a fixed-height list that scrolls instead of growing. Every row keeps its own "Open in Search Console" door, however many there are — a thousand-page site with hundreds of unindexed articles costs nothing to open and nothing stays out of reach. Assistants reading over MCP can walk the same lists page by page.
