...
* **Connect by approving, not by pasting keys** — give an assistant that address and it asks *you* for permission on a consent page on your own site, where you choose **Read only** or **Read and write** — you may grant less than it requested. Each approved assistant gets its own key and its own Disconnect. Standards-based (OAuth 2.1 with PKCE); nothing is brokered by a third party. For clients that can't ask — ChatGPT/Codex today — one revocable **shared token** does the same job, and a read-only key is shown read-only tools.
...
* **Agent Access** — the other side of the log: who *authenticates to and acts on* the machine surface Agentimus creates (*More → Agent Access*): assistants approved, keys created, used, renamed or revoked; abilities run; requests refused — each row naming the door it came through. A record, not a guard — no IP logging, it names the key used, not the person.
...
* **XML sitemap** — with no SEO plugin, Agentimus serves your sitemap (index + paginated sub-sitemaps, with last-changed dates) and advertises it in robots.txt and llms.txt; with one installed, it links theirs instead.
...
The content signals above (JSON-LD, robots, llms.txt, markdown) are read by search engines and AI tools **today**; the discovery document is **forward-looking and standards-aligned**, preparing your site for agents as they adopt these conventions. The plugin works fully whether or not anything consumes it.
...
2. Settings — a tidy, tabbed control panel; the Discovery section gives you a toggle for each agent-readiness signal, cards for Topics for AI and the per-page AI description, the search basics Agentimus covers itself when no SEO plugin is installed (per-page SEO titles, social share cards with a default image, canonical links), the weekly email's day-and-time schedule, plus experimental browser tools (WebMCP) that let an in-browser AI agent call your site search.
3. Readiness report — a plain-English pass/warn checklist of what's enabled and what's still missing, and beneath it the Optimize worklist: exactly which posts and pages an answer engine would struggle to quote, and why. Set aside anything that isn't meant to be cited — page by page or a whole check at once — and the set-aside list remembers what each page was flagged for.
...
6. Activity to review — a nav-bar alert surfaces new, high-volume or deceptive clients from any screen. Genuine ones you Allow or Block by name in one click; a client caught lying — a wrong network, or a forged Web Bot Auth signature — can't be trusted by name, so its card says exactly how it was caught ("it signed its request as chatgpt.com — but the signature didn't match chatgpt.com's published key") and, when enforcement turned it away, that it got nothing. New clients leave the queue by themselves after 48 hours; flagged ones stay until you decide. No IP logging by default; an optional setting can store IPs for flagged crawlers only.
...
10. In the post editor — Topics for AI: say in plain words what a page is about, one chip at a time, or leave it blank and let Agentimus fill them in from the post's tags and categories (those arrive marked *auto*); either way the topics flow into the page's JSON-LD keywords and its .md edition. Where you've set up an AI provider in WordPress, "Suggest with AI" drafts them from the page itself. Nothing shows to visitors. (On a site where Agentimus also provides the SEO title, these rows live in the "Search & AI" box.)
...
13. In the post editor — the "Search & AI" box: a per-page SEO title (used for search results and browser tabs when no SEO plugin is active — leave it blank and the post's own title serves), the one-line AI description that feeds the page's structured data, its .md edition and its meta description, and the topics below. Each text field falls back honestly — the description to the page's own opening words, shown right there — and "Draft with AI" writes either from the page itself.
14. Request Log — every request an agent made, in one filterable table: narrow by client, endpoint, network, verification verdict, User-Agent or date to see exactly what a single bot fetched. A Status column carries one honest mark per request — verified, signed (cryptographically proven), spoofed, forged, refused, or unchecked — with the explanation on hover, and every "Requested at" renders in your site's own date and time format. Repeat hits are grouped, and your own logged-in visits are never recorded. Records are kept for the last 30 days (or until the size cap), then trimmed — so read a full page as a floor, not a total.
...
17. Agent Access — the other side of the log: who authenticates to, and *acts* on, the machine surface Agentimus creates, as a card feed with the subject up front. Application passwords being created (a brand-new one is worth a second look — it keeps working even after you change your password), first used, renamed or revoked; WordPress abilities being run; and requests that were refused, or that probed for abilities that don't exist. The header sums what the watching has seen — events recorded, how many of the site's abilities have been used, the last activity. A record, not a guard — it never blocks — and with no IP logging, it names the key that was used, not the person.
...
20. MCP server — one switch runs a Model Context Protocol server on your own site. Your server address leads: give it to an assistant and it asks you for approval — no keys to paste — and every approved assistant is listed under **Connected assistants** with its scope, when it last called, and its own Disconnect. Setup steps fold away and show only the assistant you pick; a shared token is there for clients that can't ask. Above it, the trust ladder: a second switch lets connected agents write (drafts, edits, categories, tags, featured images, AI topics and descriptions, Readiness fixes), and a third decides whether they may publish or only leave drafts for your review.
...
25. Connecting an assistant — the consent page your own site serves when an AI assistant asks for access. It names what the assistant calls itself and where it will send you back, and you choose what it may do: **Read only**, or **Read and write** within your existing write settings — you can grant less than it asked for. Approving gives that one assistant its own key, listed under Connected assistants with its own Disconnect; no keys are copied, and nothing is brokered by anyone but your site.
