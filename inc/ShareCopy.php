...
(function(){
  ...
  function card(net, ctx){
    ...
    // The card's mark — the Sharing tab's grammar, carried here so the two
    // surfaces read as one feature.
    // ⛔⛔ THE REAL VENDOR MARK WHERE WE HOLD ONE, A MONOGRAM ONLY WHERE WE DO
    // NOT. This said "Letters, never logos" and drew "X"/"In"/"Tg" for the three
    // networks Integrations → Sharing renders with <BrandMark> — so one feature
    // wore two different marks depending on which screen you opened, and the
    // comment justified it with a claim about the Sharing tab that was not true
    // of the Sharing tab.
    // ⭐ Paths are the SAME ones resources/admin/js/brandMarks.js holds, viewBox
    // included — copied, not redrawn, so the two surfaces cannot drift. ⛔ If a
    // mark is corrected there, correct it here in the same commit.
    // ⭐ facebook / whatsapp / reddit keep their letters HONESTLY: brandMarks.js
    // holds no faithful mark for them, and an invented lookalike is the one thing
    // that is never allowed. Adding a mark there means adding it in both places.
    var LOGOS = {
      x: { vb: '-4.44 -4.44 32.88 32.88', d: ['M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z'] },
      linkedin: { vb: '-0.88 -0.78 24.76 24.76', d: ['M3.6 1.2a2.4 2.4 0 1 1 0 4.8 2.4 2.4 0 0 1 0-4.8Z','M1.6 8h4v14h-4z','M8.2 8H12v1.9h.05c.6-1.15 2.05-2.2 4.15-2.2 3.5 0 5.6 2.15 5.6 6.25V22h-4v-6.9c0-1.95-.75-3.15-2.4-3.15-1.35 0-2.1.9-2.45 1.8-.13.32-.15.75-.15 1.2V22h-4V8Z'] },
      telegram: { vb: '-1.95 -1.95 27.91 27.91', d: ['M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z'] },
      facebook: { vb: '-1.95 -1.95 27.91 27.91', d: ['M9.101 23.691v-7.98H6.627v-3.667h2.474v-1.58c0-4.085 1.848-5.978 5.858-5.978.401 0 .955.042 1.468.103a8.68 8.68 0 0 1 1.141.195v3.325a8.623 8.623 0 0 0-.653-.036 26.805 26.805 0 0 0-.733-.009c-.707 0-1.259.096-1.675.309a1.686 1.686 0 0 0-.679.622c-.258.42-.374.995-.374 1.752v1.297h3.919l-.386 2.103-.287 1.564h-3.246v8.245C19.396 23.238 24 18.179 24 12.044c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.628 3.874 10.35 9.101 11.647Z'] },
      whatsapp: { vb: '-1.95 -1.95 27.91 27.91', d: ['M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z'] },
      reddit: { vb: '-1.95 -1.95 27.91 27.91', d: ['M12 0C5.373 0 0 5.373 0 12c0 3.314 1.343 6.314 3.515 8.485l-2.286 2.286C.775 23.225 1.097 24 1.738 24H12c6.627 0 12-5.373 12-12S18.627 0 12 0Zm4.388 3.199c1.104 0 1.999.895 1.999 1.999 0 1.105-.895 2-1.999 2-.946 0-1.739-.657-1.947-1.539v.002c-1.147.162-2.032 1.15-2.032 2.341v.007c1.776.067 3.4.567 4.686 1.363.473-.363 1.064-.58 1.707-.58 1.547 0 2.802 1.254 2.802 2.802 0 1.117-.655 2.081-1.601 2.531-.088 3.256-3.637 5.876-7.997 5.876-4.361 0-7.905-2.617-7.998-5.87-.954-.447-1.614-1.415-1.614-2.538 0-1.548 1.255-2.802 2.803-2.802.645 0 1.239.218 1.712.585 1.275-.79 2.881-1.291 4.64-1.365v-.01c0-1.663 1.263-3.034 2.88-3.207.188-.911.993-1.595 1.959-1.595Zm-8.085 8.376c-.784 0-1.459.78-1.506 1.797-.047 1.016.64 1.429 1.426 1.429.786 0 1.371-.369 1.418-1.385.047-1.017-.553-1.841-1.338-1.841Zm7.406 0c-.786 0-1.385.824-1.338 1.841.047 1.017.634 1.385 1.418 1.385.785 0 1.473-.413 1.426-1.429-.046-1.017-.721-1.797-1.506-1.797Zm-3.703 4.013c-.974 0-1.907.048-2.77.135-.147.015-.241.168-.183.305.483 1.154 1.622 1.964 2.953 1.964 1.33 0 2.47-.81 2.953-1.964.057-.137-.037-.29-.184-.305-.863-.087-1.795-.135-2.769-.135Z'] }
    };
    // ⛔ EMPTY, and that is the point: every network this card offers now has a
    // faithful mark, so nothing falls back to letters. A monogram stays the
    // honest answer for a brand we hold no mark for — leave this here so the next
    // network added has somewhere to land rather than getting an invented logo.
    var MARKS = {};
    function markHtml(key, label){
      var L = LOGOS[key];
      if (L) {
        return '<svg viewBox="' + L.vb + '" width="14" height="14" fill="currentColor" aria-hidden="true">' +
          L.d.map(function(d){ return '<path d="' + d + '"/>'; }).join('') + '</svg>';
      }
      return esc(MARKS[key] || label.charAt(0));
    }
    ...
    item.innerHTML =
      '<div class="agentimus-sc__head">' +
        '<span class="agentimus-sc__mark' + (LOGOS[net.key] ? ' is-brand' : '') + '" aria-hidden="true">' + markHtml(net.key, net.label) + '</span>' +
...
JS;
	...
	public static function css() {
		...
		return '.agentimus-sc,.agentimus-sc-pop{--asc-ground:#f6f5f2;--asc-card:#fdfcfa;--asc-line:#d5d3cf;--asc-line-soft:#e7e5e0;--asc-ink:#1d2327;--asc-soft:#646970;--asc-faint:#8c8f94;--asc-field:#fff;--asc-accent:#2271b1;--asc-bad:#b32d2e;'
			...
			// A real vendor mark is a SHAPE, not a letter: it takes the card's ink
			// rather than the monogram's soft grey, or the logo reads as disabled
			// beside the network name it belongs to.
			. '.agentimus-sc__mark.is-brand{color:var(--asc-ink)}'
			. '.agentimus-sc__mark svg{display:block}'
