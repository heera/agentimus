	...
	public function run_poll() {
		...
		if ( ! $saved['ok'] ) {
			...
			if ( $saved['written'] > 0 ) {
				...
				// class that caused this bug. Progress wants the plugin's CORE
				// settings (the set-aside lists); handing it a connection store
				// threw a TypeError that WordPress showed as "there has been a
				// critical error".
				//
