...
final class Content {
	...
	/**
	 * The post types Agentomatic actually exposes: the configured selection
	...
	public static function post_types() {
		 ...
		 */
		$types = (array) apply_filters( 'agentomatic_post_types', $types, $available );
	...
	public static function estimate_full_size( Settings $settings ) {
		...
		$avg_bytes    = max( 1, (int) apply_filters( 'agentomatic_llms_full_avg_item_bytes', 4096 ) );
	...
	public static function source( $post_type ) {
		 ...
		 * @param string $post_type Post type slug.
		 */
		return (string) apply_filters( 'agentomatic_post_type_source', $source, $post_type );
	}
	 ...
	 * renderers) can short-circuit with `agentomatic_markdown_source`;
	...
	public static function markdown_source( $post ) {
		 ...
		 * @param \WP_Post     $post Post.
		 */
		$html = apply_filters( 'agentomatic_markdown_source', null, $post );
		if ( null === $html ) {
	...
	public static function has_body( $post ) {
		...
		return null !== apply_filters( 'agentomatic_markdown_source', null, $post );
	}
}
