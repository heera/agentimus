...
final class Envelope {
	...
	private function documents() {
		$docs = array(
			...
			// How agents authenticate here — generated from live config (always served).
	...
	private function well_known_index() {
		...
		foreach ( array( 'discovery.json', 'agent-card.json', 'agent.json', 'mcp.json', 'openapi.json', 'api-catalog', 'agent-skills' ) as $name ) {
		...
		// RFC 9728 protected-resource metadata is listed when any auth server backs
		// it — the owner-declared external one, or the built-in OAuth server that
		// ships with the MCP server (the flat doc serves either; see WellKnownDocs).
		if ( '' !== trim( (string) $this->settings->get( 'oauth_auth_server', '' ) ) || $this->settings->enabled( 'enable_mcp_server' ) ) {
	...
	/**
	 * The site packaged as an agentskills.io Agent Skill, at
	 * /.well-known/agent-skills/{name}/SKILL.md.
	 *
	 * @return string Markdown.
	 */
	public function skill_md() {
		return ( new SkillMd( $this->settings, $this ) )->markdown();
	}

	/**
	 * The markdown listing at the bare /.well-known/agent-skills path.
	 *
	 * @return string Markdown.
	 */
	public function skill_directory_md() {
		return ( new SkillMd( $this->settings, $this ) )->directory_markdown();
	}
