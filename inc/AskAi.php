 ...
 * came back.
 *
 * Gated by the `enable_ask_ai` setting (Settings → Content) — ON by default:
 * the bar is plain links with no script, and nothing is sent anywhere until a
 * reader clicks one. Post types are chosen via the `agentimus_ask_ai_post_types`
 * filter (default: posts only). The few style rules print inline, once per page
 * — a separate stylesheet request would cost more than it saves at this size.
 *
...
final class AskAi {
	...
	/**
	 * @param Settings $settings Settings store — carries the enable_ask_ai gate.
	 */
	...
	public function append( $content ) {
		if ( ! $this->settings->enabled( 'enable_ask_ai' ) ) {
			return $content;
		}
		...
		/** @var bool $enabled Per-post override, on top of the enable_ask_ai setting. */
	...
	private function networks() {
		$networks = array(
			...
			array(
				...
				// Google fused reading and training into ONE robots token: disallowing
				// Google-Extended (the anti-training default) also forbids Gemini/AI-Mode
				// from READING the page — so on a site that blocks it, this button can
				// only ever produce "page inaccessible". Watched live, 2026-07-27.
		 ...
		 */
		$networks = apply_filters( 'agentimus_ask_ai_networks', $networks );

		return array_values( array_filter( $networks, array( $this, 'permitted' ) ) );
	}

	/**
	 * Whether the site's OWN policy permits this assistant's button.
	 *
	 * Don't hand readers a button the site itself breaks: an entry is dropped
	 * when one of its `agents` tokens — the agent that would do the READING —
	 * is on the site's blocklists. Deliberately narrow: blocking a TRAINING
	 * crawler (GPTBot, ClaudeBot) hides nothing, because OpenAI and Anthropic
	 * fetch reader-initiated requests with separate agents (ChatGPT-User,
	 * Claude-User) that such a block never touches. Google is the exception —
	 * one token, Google-Extended, governs both training and AI-Mode reading —
	 * which is why its entry lists it. The always-allow list wins, as it does
	 * everywhere else in the plugin.
	 *
	 * @param array $network One networks() entry.
	 * @return bool
	 */
	private function permitted( array $network ) {
		if ( empty( $network['agents'] ) ) {
			return true; // A filter-added custom entry states no policy tokens.
		}

		$blocked = array_merge(
			(array) $this->settings->get( 'blocked_trainers', array() ),
			(array) $this->settings->get( 'blocked_agents', array() )
		);
		$allowed = (array) $this->settings->get( 'allowed_agents', array() );

		foreach ( (array) $network['agents'] as $token ) {
			foreach ( $allowed as $trust ) {
				if ( '' !== (string) $trust && false !== stripos( $token, (string) $trust ) ) {
					continue 2; // Trusted by name — a block elsewhere never beats an explicit allow.
				}
			}
			foreach ( $blocked as $deny ) {
				if ( '' !== (string) $deny && false !== stripos( $token, (string) $deny ) ) {
					return false;
				}
			}
		}
