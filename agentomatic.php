<?php
/**
 * Plugin Name:       Agentomatic
 * Plugin URI:        https://github.com/heera/agentomatic
...
define( 'AGENTOMATIC_VERSION', '1.0.0' );
define( 'AGENTOMATIC_FILE', __FILE__ );
define( 'AGENTOMATIC_DIR', plugin_dir_path( __FILE__ ) );
define( 'AGENTOMATIC_URL', plugin_dir_url( __FILE__ ) );

/**
 * The public registration actions the discovery standard is built on.
 * AGENTOMATIC_CANONICAL_HOOK is the canonical, vendor-neutral hook of the WP_Discovery
 * protocol; AGENTOMATIC_ALIAS_HOOK is a back-compat alias the engine also fires.
 * Providers SHOULD hook the canonical name.
 */
define( 'AGENTOMATIC_CANONICAL_HOOK', 'wpdiscovery_register' );
define( 'AGENTOMATIC_ALIAS_HOOK', 'agentomatic_register' );

/**
 * Minimal PSR-4 autoloader: Agentomatic\Foo\Bar → inc/Foo/Bar.php.
 *
...
spl_autoload_register(
	static function ( $class ) {
		...
		$path     = AGENTOMATIC_DIR . 'inc/' . str_replace( '\\', '/', $relative ) . '.php';
		if ( is_readable( $path ) ) {
			require $path;
		}
	}
...
require AGENTOMATIC_DIR . 'inc/discovery-api.php';
